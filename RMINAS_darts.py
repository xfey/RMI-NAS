...
import sampler.sampling_darts as sampling
...
from search_space.darts.darts_cnn import AugmentCNN, geno_from_alpha, reformat_DARTS


LOSS_BETA = 0.80
OBSERVE_EPO = [200, 250]    # using the last as training epoch.
RF_WARMUP = 100
RF_THRESRATE = 0.05
RF_SUCC = 100
...
def main():
    ...
    print(DATASET)
    # assert DATASET in ['cifar10', 'cifar100'], 'dataset error'
    assert DATASET in ['cifar10', 'cifar100', 'imagenet'], 'dataset error'
    if DATASET == 'imagenet':
        print('='*30+' NOTE '+'='*30)
    ...
    if DATASET == 'cifar10':
        ...
        """ResNet codes"""
        net = resnet101()
        net.load_state_dict(torch.load('./teacher_model/resnet101_cifar100/resnet101.pth'))
        net.cuda()
        ...
        NUM_CLASSES = 1000
        from utils.loader import imagenet_data
        import teacher_model.fbresnet_imagenet.fbresnet as fbresnet
        """Data preparing"""
    ...
    RFS = RF_suggest(space='darts', logger=logger, thres_rate=RF_THRESRATE, seed=cfg.RNG_SEED)
